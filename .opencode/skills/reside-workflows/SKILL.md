...
enforcement:
  files:
    ...
    - "replicas/*/src/definitions/activities.ts"
---

# ReSide Temporal Workflow and Activity Rules
...
- Model names are derived from function names as `{X}Input` and `{X}Output` for function `X`.
- If an input or output object would be empty, omit that model entirely.
- Do not use replica-name prefixes for Temporal identifiers such as workflow IDs, signal names, query names, or update names.
...
- In `definitions/activities.ts`, place models such as `*Input`, `*Output`, and helper aliases first, and activity object types last.
...
- If activity logic belongs to the whole replica rather than a narrower domain, use the replica name as the feature name.
...
- Re-exports of common workflow helpers such as `deliverOperationCompletionWorkflow` must be declared in `src/workflows/index.ts`, not in feature workflow files.
...
- If an activity returns any data, the return type must be an object (`XOutput`), never a primitive, array, or union directly.
- Activity implementations must be exported as factory functions.
- Activity factory arguments must always be destructured in the function signature.
...
- Methods inside the returned activities object must not explicitly annotate argument or return types.
...
- Keep spacing clear between documented fields and avoid tightly packed field blocks.
...
- Build formatted notification/message content in workflow or activity code via message helpers such as `block`, `inline`, and `bold`.
- Pass helper results directly to `sendNotification`, for example `message: block(...)`.
- Do not pass `.html`, for example do not use `message: block(...).html`.
- Notifications sent by workflow or business logic as system-originated events must set `system: true` in `sendNotification` calls.
- Tests must not target workflow functions directly.
- Testing Temporal functionality at feature, business, or service layers is allowed.
