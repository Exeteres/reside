...
export function createOperationActivities(operationService: OperationActivitiesService) {
  return {
    subscribeToOperationCompletion: async (operationId: number, workflowId: string) => {
      const callbackEndpoint = `${getReplicaEndpoint()}:80`
      ...
      logger.info(
        ...
        "subscribing to operation completion from activity",
      )
      ...
      logger.info(
        {
          ...
          completedStatus:
            response.response.case === "completedOperation"
              ? response.response.value.status
              : undefined,
        },
        "operation completion subscribe response",
      )
