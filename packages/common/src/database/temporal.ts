...
export type WorkerOptions = {
  ...
  /**
   * Optional explicit Temporal credentials.
   *
   * When provided, worker startup does not request credentials from infra provision service.
   */
  temporalCredentials?: {
    address: string
    namespace: string
...
async function createTemporalWorker(options: WorkerOptions): Promise<Worker> {
  try {
    const credentials =
      options.temporalCredentials ?? (await getTemporalCredentials(options.services))
