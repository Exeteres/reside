...
export function createRequirementCore<TContract extends Contract>(
  contract: TContract,
  account: ReplicaAccount<any, any>,
): Requirement<TContract> {
  if (!account.profile.$isLoaded) {
    throw new Error(`Account profile for account ID "${account.$jazz.id}" is not loaded`)
  }
  ...
  const methods = mapValues(contract.methods, (method, methodName) => {
    const send: ReturnType<typeof method.definition>["send"] = async (requestData, options) => {
      if (!endpoint) {
        throw new Error(
          `No ${context} endpoint defined for replica "${replicaName}" (account ID: ${account.$jazz.id})`,
        )
      }
      ...
      const definition = method.definition(fullUrl, account.$jazz.id)
  ...
  return {
    ...
    account,
    ...methods,
    checkMyPermission: checkPermission,
...
export async function createRequirement<TContract extends Contract>(
  ...
  return createRequirementCore(contract, account as ReplicaAccount<any, any>)
}
