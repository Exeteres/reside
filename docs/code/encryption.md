...
This document defines mandatory rules for storing and handling personal information in ReSide code.
...
Messages sent through `sendNotification` will automatically replace every ECID found in the message context with the decrypted content for the final external notification.
...
- RHIDs are one-way hashes and will not, and cannot, be substituted with plaintext.

## Business logic

- Business functions that need encryption must accept `ResideCrypto` as an explicit dependency.
- Pass `ResideCrypto` as a dedicated positional argument, following `docs/code/business.md`.
...
- Follow `docs/code/logging.md` and additionally treat plaintext personal information as forbidden log data.
