# TypeScript General Style Guide

This document defines general TypeScript stylistic rules for ReSide.

Specialized rules are documented separately:

- Type rules: `docs/code/types.md`
- Error rules: `docs/code/errors.md`
- Logging rules: `docs/code/logging.md`
- Service rules: `docs/code/services.md`
- Testing rules: `docs/code/testing.md`
...
- Don't use `@returns` tag for `void` functions or `Promise<void>` functions, just end the description without it.
- Private methods must not define JSDoc. If method has `export` keyword but is not exported from the package, it is still considered private and should not have JSDoc.
