...
await setupLanguageSubsystem({
  ...
  instructions:
    "Помогай пользователям управлять валютой «нихуя» (∅): показывать баланс, историю, выполнять переводы и предоставлять аудит безопасности банковской реплики. Используй только RHID субъекта из контекста или явно переданные opaque RHID. Не раскрывай внутренние зашифрованные значения.",
