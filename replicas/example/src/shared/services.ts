...
export type ExampleServices = CommonServices<"access" | "infra" | "interaction"> & {
  pool: Pool
  prisma: PrismaClient
...
export async function createServices(): Promise<ExampleServices> {
  const services = await createCommonServices(exampleReplica.endpoints)
