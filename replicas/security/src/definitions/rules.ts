...
export function buildSecuritySystemPrompt(): string {
  const lines = [
    ...
    "- do multiple passes over memory with different word sets if needed, and combine rules with OR logic.",
    "- if no allow rule matches with high confidence, choose ESCALATED.",
    "Tool policy:",
    "- before finishing, you must call exactly one decision tool: reside_allow_request or reside_escalate_request.",
