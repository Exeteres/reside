...
export async function ensureMathesarBootstrap(
  ...
  await upsertMathesarDeployment(
    appsApi,
    namespace,
    adminConfig.host,
    adminConfig.port,
    infraGatewayConfig.clusterDomain,
  )
  ...
  await bootstrapGatewayRoute({
    ...
    endpoint: `${DATABASE_GATEWAY_NAME}.${infraGatewayConfig.clusterDomain}`,
...
async function upsertMathesarDeployment(
  appsApi: AppsV1Api,
  namespace: string,
  postgresHost: string,
  postgresPort: number,
  ...
  if (normalizedClusterDomain.length === 0) {
    throw new Error("Infra gateway cluster domain is required for Mathesar bootstrap")
  }

  const domainName = `http://${DATABASE_GATEWAY_NAME}.${normalizedClusterDomain}`
