...
export function createImplementationPrompt(
  ...
  return [
    ...
    "When migration generation, Prisma checks, or database-backed tests need a development database, call reside_create_dev_database and use the returned DATABASE_URL with direct commands such as `DATABASE_URL=... bun prisma migrate dev`.",
    "Temporary development databases are removed after 24 hours. If a resumed session finds that a created database disappeared, call reside_create_dev_database again and continue with the new DATABASE_URL.",
    ...
    "Prefer reside_commit_changes for normal commits because it stages paths, creates a conventional commit without a body, and validates branch commit rules. Raw git commit commands remain available for recovery and advanced history fixes.",
    "Before calling reside_deliver_changes, ensure git HEAD is on the initial branch shown above.",
    ...
    "If reside_deliver_changes fails with commit validation, rewrite invalid commit message(s) first (at minimum amend latest commit), then retry reside_deliver_changes.",
    ...
    "Before calling reside_deploy_replica, commit your changes. If you are confident deploy is safe without PR, you may deploy directly.",
